...
WHY THIS EXISTS
    kern's seccomp filter is moving from a denylist (allow everything except 34 named syscalls) to an
    ALLOWLIST (deny everything except a vetted set). The vetted set is Docker/moby's default profile,
    which is validated by billions of container starts, MINUS the 34 kern denies for being rootless
...
# The 34 kern denies regardless of the OCI profile (kill / ENOSYS / nesting). They must NOT enter the
...
KERN_DENIED = {
    ...
    "perf_event_open", "request_key", "syslog", "userfaultfd",
}
...
def sidecar_text(names: list[str]) -> str:
    ...
    header = (
        ...
        "# (moby-default-v27.3.1.json) MINUS the 34 syscalls kern denies for being rootless. The\n"
...
def rust_text(names: list[str]) -> tuple[str, list[str]]:
    ...
    header = (
        ...
        "// (crates/kern-isolation/seccomp/moby-default-v27.3.1.json) MINUS the 34 syscalls kern denies\n"
