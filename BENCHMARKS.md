...
| | |
|---|---:|
| **kern** binary | **1.86 MB** x86_64, **1.57 MB** aarch64 from a plain `cargo build --release` (the size-optimized release build is **1.58 MB / 1.31 MB**), static and stripped, one Rust dependency (`libc`); OCI pull shells out to the system `curl`/`tar` |
