...
//! not supervise: a service that dies an hour later is not restarted by it, because kern has no
//! per-stack supervisor yet. The emitted unit says so in a comment rather than letting the reader
//! infer a guarantee that is not there.
//!
...
pub fn render_unit(spec: &UnitSpec<'_>) -> Result<String, Error> {
    ...
    Ok(format!(
        "# Generated by `kern compose {file} systemd`. Not installed: write it where you want it.\n\
         ...
         # a service that dies later is not restarted, because kern has no per-stack supervisor yet.\n\
         {linger}#\n\
         # Install ({scope_word}):\n\
...
mod tests {
    ...
    fn the_unit_models_a_command_that_exits() {
        ...
        // No supervision is claimed anywhere.
        assert!(
