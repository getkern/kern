...
{b}COMMANDS:{z}
  ...
  {d}Multi-box{z}
    ...
    {c}pod{z} create <name> [--no-outbound] [--no-uid-range] [--bridge <cidr>]  Shared-network pod: peers reach each other by name.
...
pub fn gc(images: bool) -> Result<(), Error> {
    ...
    // resolves, nothing refreshes and no other sweep collects. Found by an independent test with
...
pub fn uninstall(yes: bool, keep_images: bool) -> Result<(), Error> {
    ...
    if let (Some(exe), Some(h)) = (std::env::current_exe().ok(), crate::global_env("HOME")) {
