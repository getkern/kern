...
**A real, kernel-enforced container in ~3.6 ms, out of one 1.59 MB binary with no daemon.**

<p align="center">
  <img src="assets/kern-demo.gif" width="720" alt="Terminal: 'kern box app --image alpine -- echo hello from a real container' prints the greeting, then reports that kern started in 3.6 ms against docker run's 290 ms. A real OCI image, rootless, a 1.59 MB binary, no daemon, on an Intel i7-14700KF, Linux 7.0.">
</p>
...
and a stack runner at once, in 1.59 MB with no daemon.
...
(1.59 MB, the one a release ships), against the runtimes installed there. Every number comes from one
...
**The core is done. Everything above works today and is tested:** 837 Rust, 78 Python and 61 Node
