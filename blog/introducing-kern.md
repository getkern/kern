...
AI-generated code. One ~1.52 MB static binary, one Rust dependency (`libc`), no daemon. It starts a
...
*local* and ~1.52 MB, no cloud, no account, no VM.
...
in ~1.52 MB. The real gap is to the *engines*: **~79-127× faster to start** than podman/Docker (the
...
you want a VM boundary, reach for a microVM, a deliberate trade for 2.3 ms starts and a ~1.52 MB
