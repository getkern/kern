...
*A fast, rootless sandbox and virtual resource runtime for your workloads, including untrusted and
...
*local* and one static binary, no cloud, no account, no VM.
...
| runtime | cold start | |
|---|---|---|
| **kern** `box --rootfs` | **2.26 ms** uncapped, 2.60 capped | overlay + self-pivot + seccomp |
| bubblewrap | 2.30 ms | a sandbox *primitive*, no images, caps, or lifecycle |
| runc (rootless) | 18.6 ms | OCI runtime (Go) |
| podman (rootless) | 292.9 ms | daemonless engine: forks `conmon` + the full OCI stack per run |
| docker run --rm | 296.9 ms | client → daemon round-trip |
...
out of one static binary. The real gap is to the *engines*: **~79-127× faster to start** than podman/Docker (the
spread is the two kern paths: 2.3 ms with `--rootfs`, ~3.5 ms with `--image`, which also maps a uid
range), which
...
you want a VM boundary, reach for a microVM, a deliberate trade for 2.3 ms starts and a
single-binary footprint. [SECURITY.md](https://github.com/getkern/kern/blob/main/SECURITY.md) marks every guarantee
